...
async fn start_rpc_server() {
    ...
    io.add_method("get_chapter_images", |params: Params| -> BoxFuture<Result<Value, Error>> {
        Box::pin(async move {
            ...
            let empty = String::new();
            let series_slug = args.get(0).unwrap_or(&empty);
            let chapter_slug = args.get(1).unwrap_or(&empty);
    ...
    io.add_method("extension_info", |_params: Params| -> BoxFuture<Result<Value, Error>> {
        Box::pin(async move {
            Ok(serde_json::to_value(qimanga::Qimanga::extension_info()).unwrap())
        })
    });
